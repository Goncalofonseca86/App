...
import { Calendar, MapPin, User, Clock, CheckCircle, LogOut } from 'lucide-react';
import { User as UserType, getRoleDisplayName, getRoleColor, hasPermission } from '../data/users';
...
export default function WorkAssignment() {
  ...
  const assignWork = (workId: string, technicianId: string) => {
    ...
    setTechnicians(prev => prev.map(tech => {
      ...
      return tech.id === technicianId
        ? { ...tech, currentWorkload: tech.currentWorkload + (work?.estimatedHours || 0) }
        : tech;
    }));
  };
    ...
    if (work?.assignedTo) {
      setTechnicians(prev => prev.map(tech =>
        tech.id === work.assignedTo
          ? { ...tech, currentWorkload: Math.max(0, tech.currentWorkload - work.estimatedHours) }
          : tech
      ));
    }
  ...
  const canOnlyViewWorks = hasPermission(currentUser, 'view_works') && !canAssignWorks && !canCreateWorks;
  ...
  return (
    <div className="leirisonda-main">
        ...
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-white mb-2">
              {canAssignWorks ? 'Atribuição de Obras' : 'Consulta de Obras'}
            </h1>
            <p className="text-white/90">
              {canAssignWorks ? 'Gerir e atribuir obras aos técnicos disponíveis' : 'Consultar e editar informações das obras'}
            </p>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/70 text-sm">{currentUser.department}</span>
              </div>
            </div>
            <Button
              variant="outline"
      ...
      <div className="content-grid">
        ...
        <div className="space-y-6">
          <div>
            <h2 className="text-xl font-semibold mb-4">
              {canAssignWorks ? 'Obras Disponíveis' : 'Todas as Obras'}
            </h2>
            <div className="space-y-4">
              {(canAssignWorks ? works.filter(work => work.status === 'pending') : works).map(work => (
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Badge className={getStatusColor(work.status)}>
                          {work.status === 'pending' ? 'Pendente' : work.status}
                        </Badge>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        Prazo: {new Date(work.deadline).toLocaleDateString('pt-PT')}
                      </div>
                    </div>

                    <div className="flex items-center gap-4">
                      {canAssignWorks ? (
                        <Select onValueChange={(techId) => assignWork(work.id, techId)}>
                          <SelectTrigger className="w-64">
                            <SelectValue placeholder="Selecionar técnico" />
                          </SelectTrigger>
                          <SelectContent>
                            {technicians
                              .filter(tech => (tech.currentWorkload + work.estimatedHours) <= tech.maxWorkload)
                              .map(tech => (
                              <SelectItem key={tech.id} value={tech.id}>
                                <div>
                                  <div className="font-medium">{tech.name}</div>
                                  <div className="text-xs text-gray-500">
                                    {tech.specialization} • {tech.currentWorkload}/{tech.maxWorkload}h
                                  </div>
                                </div>
                              </SelectItem>
                            ))}
          ...
          {/* Obras Atribuídas - só para quem pode atribuir */}
          {canAssignWorks && (
          <div>
            <h2 className="text-xl font-semibold mb-4">Obras Atribuídas</h2>
            <div className="space-y-4">
              {works.filter(work => work.status === 'assigned').map(work => {
                const assignedTech = technicians.find(t => t.id === work.assignedTo);
                return (
                  <Card key={work.id} className="card-leirisonda border-blue-200">
                    <CardHeader>
                      <div className="flex justify-between items-start">
                        <div>
                          <CardTitle className="text-lg">{work.title}</CardTitle>
                      ...
                      {assignedTech && (
                        <div className="flex items-center justify-between bg-blue-50 p-3 rounded-lg">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-medium text-blue-900">{assignedTech.name}</div>
                              <div className="text-sm text-blue-700">{assignedTech.specialization}</div>
                            </div>
                          </div>
                          {canAssignWorks && (
                            <Button
        ...
        {/* Lista de Técnicos - só para quem pode atribuir */}
        {canAssignWorks && (
        <div>
          <h2 className="text-xl font-semibold mb-4">Técnicos Disponíveis</h2>
          <div className="space-y-4">
            {technicians.map(tech => {
              const workloadPercentage = getWorkloadPercentage(tech.currentWorkload, tech.maxWorkload);
              const isOverloaded = workloadPercentage >= 100;

              return (
                <Card key={tech.id} className={`card-leirisonda ${isOverloaded ? 'border-red-200' : ''}`}>
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <div>
                      ...
                      <Badge className={isOverloaded ? 'bg-red-100 text-red-800 border-red-200' : 'bg-green-100 text-green-800 border-green-200'}>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span>{tech.currentWorkload}/{tech.maxWorkload}h</span>
                        </div>
